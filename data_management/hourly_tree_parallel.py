...
def flush_messages(messages):
    """Print all messages in the buffer at once and clear the buffer."""
    if messages:
        print("\n".join(messages))
        messages.clear()  # Clear the buffer after printing


def extract_tar(tar_file, destination, messages):
    """Extract a tar file to the specified destination."""
    with tarfile.open(tar_file, "r") as tar:
        tar.extractall(path=destination)
    messages.append(f"Extracted {tar_file} to {destination}")


def move_file_to_hour_dir(fp, day_dir, messages):
    ...
    fp.rename(new_path)
    messages.append(f"Moved {fp.name} to {new_path}")
    return new_path
...
def untar_and_move_files(y, m, d, tars_dir):
    ...
    messages = []  # Initialize message buffer
    ...
    if day_dir.exists():
        ...
        if nfiles_dir < nfiles_tar:
            messages.append(f"Reprocessing day dir {day_dir.as_posix()}")
            ...
            if fpaths:
                ...
                with ThreadPoolExecutor() as executor:
                    executor.map(lambda fp: move_file_to_hour_dir(fp, day_dir, messages), fpaths)
                flush_messages(messages)
            else:
                messages.append(f"No FITS files found in {day_dir}")

    elif not day_dir.exists() and tar_file.is_file():
        messages.append(f"Processing: {tar_file}")
        # Untar the file (CPU-bound, so this runs in a separate process)
        extract_tar(tar_file, tars_dir, messages)
        ...
        messages.append(f"Created hourly directories in {day_dir}")
    ...
    # Final flush to ensure all messages are printed
    flush_messages(messages)
